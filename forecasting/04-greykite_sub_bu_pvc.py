...
import yaml
from greykite_sub_bu_pvc_query import *
...
# busines sunits
sub_bus = conf['sub_bu']['pvc']

# on the fly setting.
# TODO: we might need to move some of these configs to the yaml file 

today = date.today()
start_date_future = (today + MonthEnd(1)).strftime('%Y-%m-%d')
...
def get_data(bu, start_date, end_date):
  ...
  spark.sql(f"drop table if exists ai.pvc_sub_bu")
  spark.sql(f"create table ai.pvc_sub_bu as ({sql_quey})")
  data = spark.sql(f"select last_day(cast(make_date(year, month, 1) as date)) as date, DOLLAR_DBU as dbu_dollar from ai.pvc_sub_bu where sub_bu = {bu} and cast(make_date(year, month, 1) as date) between {start_date} and {end_date} \
    union all \
...
class prep_results ():
  ...
  def run(self):
    ...
    .mode("append")
    .saveAsTable(f"{temp_database}.{output_table_name}_sub_bu_pvc"))
...
# if spark.sql(f"select count(*) from {dollar_dbu_pvc_database}.{dollar_dbu_pvc_table[0]} where month = {12 if today.month==1 else today.month-1} and year = {today.year-1 if today.month==1 else today.year}").first()[0] != 0:
for bu in sub_bus:
...
# MAGIC %sql
# MAGIC INSERT INTO usage_forecasting_prod.forecast_results_pvc SELECT * FROM 
# MAGIC (
# MAGIC WITH CLEAN_PROD_PVC_DATA AS (
# MAGIC   SELECT
# MAGIC     date_add(last_day(add_months(ts, -1)),1) as ts,
# MAGIC     replace(region, "'", "") as region,
# MAGIC     replace(segment, "'", "") as segment,
# MAGIC     forecast,
# MAGIC     forecast_lower,
# MAGIC     forecast_upper,
# MAGIC     `timestamp`
# MAGIC   from
# MAGIC     ai_2.pvc_temp
# MAGIC )
# MAGIC SELECT
# MAGIC   concat(a.region, a.segment, 'y') as ts_key,
# MAGIC   cast(ts as date) as date,
# MAGIC   'NA' as train_run_uuid,
# MAGIC   cast(`timestamp` as date) AS train_run_date,
# MAGIC   cast(`timestamp` as date) AS score_run_date,
# MAGIC   add_months(forecast_as_of, 1) as forecast_as_of,
# MAGIC   forecast as y,
# MAGIC   forecast_lower as y_lower,
# MAGIC   forecast_upper as y_upper,
# MAGIC   'true' as is_active
# MAGIC from
# MAGIC   CLEAN_PROD_PVC_DATA as a
...
# MAGIC   on a.region = b.region and a.segment = b.segment
# MAGIC );
# MAGIC Update usage_forecasting_prod.forecast_results_pvc set train_run_uuid = NULL;
# MAGIC drop table if exists ai_2.pvc_temp;
